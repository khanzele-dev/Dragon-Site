...
const bodySchema = z.object({ subscriptionKey: z.string().min(1), planId: z.string() })

// Открытый эндпоинт (без сессии) — лимитируем по IP.
export const POST = withErrorHandling(async (req: NextRequest) => {
  const ip = clientIp(req)
  if (await isRateLimited(`guest:renew:ip:${ip}`, 10, 5 * 60 * 1000)) {
  ...
  let found
  try {
    found = await getRemnawaveUserByShortUuid(key)
  } catch (err) {
    console.error("[guest/payments/renew] lookup failed", err)
